...
async function ensureAvatarBucket() {
  ...
  if (Array.isArray(buckets) && buckets.some((bucket: any) => bucket.name === AVATAR_BUCKET)) {
    return;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file.type.startsWith('image/')) {
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: 'Avatar must be an image file' } },
        { status: 400 }
      );
    }
    ...
    if (file.size > MAX_AVATAR_SIZE_BYTES) {
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: 'Avatar image must be under 5MB' } },
        { status: 400 }
      );
    }
