...
const MAX_PROJECT_IMAGE_SIZE_BYTES = 8 * 1024 * 1024;
...
async function ensureBucket() {
  ...
  if (Array.isArray(buckets) && buckets.some((bucket: any) => bucket.name === PROJECT_IMAGE_BUCKET)) {
    return;
  }

  const { error: createError } = await supabase.storage.createBucket(PROJECT_IMAGE_BUCKET, {
    public: true,
    fileSizeLimit: `${MAX_PROJECT_IMAGE_SIZE_BYTES}`,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(file instanceof File)) {
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: 'Project image file is required' } },
        { status: 400 }
      );
    }
    ...
    if (!file.type.startsWith('image/')) {
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: 'Project attachment must be an image file' } },
        { status: 400 }
      );
    }
    ...
    if (file.size > MAX_PROJECT_IMAGE_SIZE_BYTES) {
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: 'Project image must be under 8MB' } },
        { status: 400 }
      );
    }
    ...
    const { data: publicUrlData } = supabase.storage.from(PROJECT_IMAGE_BUCKET).getPublicUrl(filePath);
